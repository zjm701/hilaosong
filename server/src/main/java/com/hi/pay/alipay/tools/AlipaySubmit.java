package com.hi.pay.alipay.tools;
...
import com.hi.pay.alipay.AlipayConfig;
import com.hi.pay.alipay.tools.httpclient.HttpProtocolHandler;
import com.hi.pay.alipay.tools.httpclient.HttpRequest;
import com.hi.pay.alipay.tools.httpclient.HttpResponse;
import com.hi.pay.alipay.tools.httpclient.HttpResultType;
import com.hi.tools.sign.MD5;
import com.hi.tools.sign.RSA;
...
public class AlipaySubmit {
	 ...
	 */
	public static String getRequestURL(String ALIPAY_GATEWAY_NEW, Map<String, String> sParaTemp) throws UnsupportedEncodingException {
		// 待请求参数数组
		Map<String, String> sPara = buildRequestPara(sParaTemp);
		List<String> keys = new ArrayList<String>(sPara.keySet());
		StringBuffer url = new StringBuffer(ALIPAY_GATEWAY_NEW + "_input_charset=" + AlipayConfig.input_charset);
		for (int i = 0; i < keys.size(); i++) {
			String name = (String) keys.get(i);
			String value = (String) sPara.get(name);
			if (value != null) {
				url.append("&" + name + "=" + URLEncoder.encode(value, AlipayConfig.input_charset));
			} else {
